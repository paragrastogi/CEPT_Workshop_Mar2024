...
for site in sites:
    for usage in simpleUsages:
        ...
        subData = data.loc[(data.loc[:,'site_id']==site) & (data.loc[:,'simpleUsage']==usage),:]

        if subData.shape[0]<=25:
            models.append(dict(location=site, btype=usage, model=LinearRegression(), rmse=np.nan, mae=np.nan))
            ...
            models.append(dict(location=site, btype=usage, model=model_no_cat, rmse=rmse_no_cat, mae=mae_no_cat))
